...
export default function Navbar() {
    ...
    // State for Mobile Off-Canvas Menus
    ...
    return (
        <>
            <header className="bg-[var(--primary-color)] shadow-sm sticky top-0 z-40 px-4">
                <div className="container mx-auto flex items-center justify-between h-16">
                    ...
                    <div className="hidden lg:flex w-full items-center justify-between">
                            ...
                            <ul className="flex space-x-6 items-center">
                                {menuItems.map((item) => (
                                    <li key={item.name} className="py-4 static" onMouseEnter={() => handleDesktopMouseEnter(item)} onMouseLeave={handleDesktopMouseLeave}>
                                        <a href={item.href} className="text-[var(--text-color)] hover:text-[var(--text-color-h)] transition inline-flex items-center">
                                            {item.name}
                                            {item.dropdownContent && (<FiChevronDown className="ml-1 mt-1 h-4 w-4" />)}
                                        </a>
                                        {hoveredMenu === item.name && item.dropdownContent && (
                                            <div className={`absolute top-full left-0 right-0 bg-[var(--primary-color)] shadow-sm border-t border-[var(--link-color)] transition-all duration-300 ease-in-out opacity-100 visible translate-y-0`}>
                                                {item.name === 'Courses' && (
                                                    <div className="container mx-auto p-6">
                                                        ...
                                                        <ul className="grid grid-cols-4 gap-x-8 gap-y-3">
                                                            {coursesMenuData['Popular Courses'].main.links.map(link => (<li key={link.name}><a href={link.href} className="text-sm text-[var(--subprimary-color)] hover:text-[var(--text-hover-color)] ">{link.name}</a></li>))}
                                                        </ul>
                                                ...
                                                {(item.name === 'University' || item.name === 'College') && (
                                                    <div className="container mx-auto flex">
                                                        <div className="w-1/4 bg-[var(--secondary-color)] text-[var(--text-color-primary)]">
                                                            <ul className="py-4">
                                                                {Object.keys(item.dropdownContent).map((subMenuName) => (
                                                                    <li
                                                                        ...
                                                                        className={`px-5 py-3 cursor-pointer text-sm font-medium transition-colors duration-200 ${activeDesktopSubMenu === subMenuName
                                                                            ? "relative bg-[var(--secondary-color)] text-white font-semibold after:content-[''] after:block after:absolute after:top-1/2 after:-translate-y-1/2 after:left-full after:w-0 after:h-0 after:border-y-[8px] after:border-l-[8px] after:border-y-transparent after:border-l-[var(--secondary-color)]"
                                                                    ...
                                                                    {Object.values(item.dropdownContent[activeDesktopSubMenu]).map(section => (
                                                                        <div key={section.title}>
                                                                            ...
                                                                            <ul className="space-y-2">
                                                                                {section.links.slice(0, 16).map(link => (<li key={link.name}><a href={link.href} className="text-sm text-[var(--subprimary-color))]  hover:text-[var(--text-hover-color)]">{link.name}</a></li>))}
                                                                            </ul>
                        ...
                        <div className="flex items-center space-x-4 flex-shrink-0">
                            <button className="text-blue-500 border border-blue-500 cursor-pointer px-4 py-2 rounded-md hover:bg-blue-50 transition font-semibold text-sm">Login/Signup</button>
                            <button onClick={toggleSearchMenu} className="text-orange-500 border border-orange-500 h-10 w-10 flex items-center justify-center rounded-md cursor-pointer hover:bg-orange-50 transition">
                                <FiSearch className="h-5 w-5" />
                            </button>
            ...
            <div className={`md:hidden fixed inset-0 z-50 transition-opacity duration-300  ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <div className="absolute inset-0 bg-[var(--secondary-color)]  bg-opacity-50" onClick={handleCloseMobileMenu}></div>
                <div className={`fixed top-0 left-0 h-full w-full max-w-sm bg-[var(--primary-color)] text-[var(--text-color-primary)] shadow-xl transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                    <div className="relative w-full h-full overflow-hidden">
                        <div className={`absolute inset-0 transition-transform duration-300 ease-in-out ${mobileSubMenu ? '-translate-x-full' : 'translate-x-0'}`}>
                            <div className="h-full flex flex-col">
                                <div className="p-4 bg-[var(--secondary-color)] text-[var(--text-color-primary)] flex items-center justify-between"><div className="flex items-center gap-3"><img src="https://placehold.co/40x40/FFFFFF/3B82F6?text=U" alt="User Avatar" className="w-10 h-10 rounded-full border-2 border-white" /><div><p className="font-semibold">Username</p><p className="text-xs opacity-80">Email@gmail.com</p></div></div><button onClick={handleCloseMobileMenu} className="p-1"><FiX className="w-5 h-5" /></button></div>
                                <div className="p-4"><button className="w-full bg-[var(--primary-color)] text-[var(--text-hover-color)] border border-[var(--text-hover-color)] px-4 py-2 rounded-md font-semibold text-sm">Login/Signup</button></div>
                                <nav className="flex-grow overflow-y-auto"><ul className="flex flex-col">{menuItems.map((item) => (<li key={item.name} className="border-b border-gray-300"><a href={item.href} onClick={(e) => handleSubMenuClick(e, item)} className="flex justify-between items-center text-[var(--primary-color-2)] p-4 transition-colors"><span>{item.name}</span>{item.dropdownContent && <FiChevronRight className="w-5 h-5 text-gray-400" />}</a></li>))}</ul></nav>
                            </div>
                        </div>
                        <div className={`absolute inset-0 transition-transform duration-300 ease-in-out bg-[var(--primary-color)] ${!mobileSubMenu ? 'translate-x-full' : 'translate-x-0'} ${mobileDetailMenu ? '-translate-x-full' : ''}`}>{mobileSubMenu && (<div className="h-full flex flex-col"><div className="p-4 bg-[var(--secondary-color)] text-[var(--text-color-primary)] flex items-center gap-4 flex-shrink-0"><button onClick={() => setMobileSubMenu(null)} className="p-1"><FiArrowLeft className="w-5 h-5" /></button><h3 className="font-semibold text-lg">{mobileSubMenu.title}</h3></div><nav className="flex-grow overflow-y-auto"><ul className="flex flex-col">{Object.entries(mobileSubMenu.data).map(([subItemName, subItemData]) => (<li key={subItemName} className="border-b border-gray-200"><a href="#" onClick={(e) => handleDetailMenuClick(e, subItemName, subItemData)} className="flex justify-between items-center text-[var(--primary-color-2)] hover:bg-gray-100 p-4 transition-colors"><span>{subItemName}</span><FiChevronRight className="w-5 h-5 text-gray-400" /></a></li>))}</ul></nav></div>)}</div>
                        <div className={`absolute inset-0 transition-transform duration-300 ease-in-out bg-[var(--primary-color)] text-[var(--primary-color-2)] ${!mobileDetailMenu ? 'translate-x-full' : 'translate-x-0'}`}>{mobileDetailMenu && (<div className="h-full flex flex-col"><div className="p-4 bg-[var(--secondary-color)] text-[var(--text-color-primary)] flex items-center gap-4 flex-shrink-0"><button onClick={() => setMobileDetailMenu(null)} className="p-1"><FiArrowLeft className="w-5 h-5" /></button><h3 className="font-semibold text-lg">{mobileDetailMenu.title}</h3></div><nav className="flex-grow overflow-y-auto p-2">{Object.entries(mobileDetailMenu.data).map(([accordionTitle, accordionContent]) => {
